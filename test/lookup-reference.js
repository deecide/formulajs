...
describe('Lookup Reference', function () {
  it('MATCH', function () {
    ...
    lookup.MATCH('jimb', ['jima', 'jimb', 'jimc', 'bernie'], 0).should.equal(2);
    lookup.MATCH('jimc', ['jima', 'jimb', 'jimc', 'bernie'], 0).should.equal(3);
    lookup
      .MATCH('jimc', ['jima', 'jimb', 'jimd', 'bernie'], -1)
      .should.equal(3);
  ...
  it('LOOKUP', function () {
    ...
    lookup
      ...
      .should.equal('yellow');
    lookup
      .LOOKUP('Jamie', ['Jim', 'Jack', 'Franck'], ['blue', 'yellow', 'red'])
      .should.equal('red');
    lookup
      .LOOKUP(
        'Jamie',
      ...
      .should.equal('red');
    lookup
      .LOOKUP(0.23, [[0.1], [0.2], [0.3], [0.4]], [['A'], ['B'], ['C'], ['D']])
      .should.equal('B');
    lookup
      .LOOKUP(0, [[0.1, 0.2, 0.3, 0.4]], [['A', 'B', 'C', 'D']])
      .should.equal(error.na);
    lookup
      .LOOKUP(0.21, [[0.1, 0.2, 0.3, 0.2]], [['A', 'B', 'C', 'D']])
      .should.equal('B');
  });
  ...
  it('INDEX', function () {
    ...
    lookup.INDEX([['Banana', 'Apple']], undefined, 2).should.equal('Apple');
  });
});
