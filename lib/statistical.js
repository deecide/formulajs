...
exports.AVEDEV = function () {
  ...
  return (
    jStat.sum(jStat(range).subtract(jStat.mean(range)).abs()[0]) / range.length
  );
};
...
exports.AVERAGEIF = function (range, criteria, average_range) {
  ...
  var tokenizedCriteria = isWildcard
    ? null
    : evalExpression.parse(criteria + "");
  ...
  for (var i = 0; i < range.length; i++) {
    ...
    if (isWildcard) {
      ...
      var tokens = [
        evalExpression.createToken(value, evalExpression.TOKEN_TYPE_LITERAL),
      ].concat(tokenizedCriteria);
...
exports.BINOM.DIST.RANGE = function (
  trials,
  probability,
  successes,
  successes2
